...
def get_mask(edg_img, mask_scale=0.6):
    ...
    ret = np.array([[(1, imgshape[0]),
                     (1, int(imgshape[0] * mask_scale)),
...
def Hough_transform(edg_img, img, mask_scale=0.3):
    # img是原始图像
...
if __name__ == '__main__':
    ...
    #                  (imgshape[1] - 1, imgshape[0] - 1)]], dtype=np.int32)
    ret = np.array(
        [[(0, imgshape[0]), (0, imgshape[0] // 2), (imgshape[1], imgshape[0] // 2), (imgshape[1], imgshape[0])]])
    cv2.fillPoly(img_gray, ret, 255)
    cv2.imshow('img_gray', img_gray)
    cv2.waitKey(0)

    edg_img = canny_edg_(img)
    cv2.imshow('edg_img', edg_img)
    cv2.waitKey(0)

    img = Hough_transform(edg_img, img, mask_scale=0.3)
    cv2.imshow('img', img)
    cv2.waitKey(0)
