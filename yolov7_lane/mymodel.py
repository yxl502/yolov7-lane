...
class YoloV7:
    # def __init__(self, yolov7_yaml_path='config/yolov5s.yaml'):
    def __init__(self, yolov7_yaml_path='cfg/training/yolov7.yaml'):
        '''初始化'''
        # 载入配置文件
        with open(yolov7_yaml_path, 'r', encoding='utf-8') as f:
            self.yolov7 = yaml.load(f.read(), Loader=yaml.SafeLoader)
        # 随机生成每个类别的颜色
        self.colors = [[np.random.randint(0, 255) for _ in range(
            3)] for class_id in range(self.yolov7['class_num'])]
        # 模型初始化
        self.init_model()
    ...
    def init_model(self):
        ...
        set_logging()
        # 选择计算设备
        device = select_device(self.yolov7['device'])
        # 如果是GPU则使用半精度浮点数 F16
        is_half = device.type != 'cpu'
        # 载入模型
        model = attempt_load(
            self.yolov7['weight'], map_location=device)  # 载入全精度浮点数的模型
        input_size = check_img_size(
            self.yolov7['input_size'], s=model.stride.max())  # 检查模型的尺寸
        if is_half:
        ...
        img_torch = torch.zeros(
            (1, 3, self.yolov7['input_size'], self.yolov7['input_size']), device=device)  # init img
        # 创建模型
        # run once
    ...
    def preprocessing(self, img):
        ...
        img_resize = letterbox(img, new_shape=(
            self.yolov7['input_size'], self.yolov7['input_size']), auto=False)[0]
    ...
    def detect(self, img, canvas=None, view_img=True):
        ...
        # NMS 非极大值抑制
        pred = non_max_suppression(pred, self.yolov7['threshold']['confidence'],
                                   self.yolov7['threshold']['iou'], classes=None, agnostic=False)
        t2 = time_sync()
        ...
        if det is not None and len(det):
            ...
            for *xyxy, conf, class_id in reversed(det):
                ...
                if view_img:
                    # 绘制矩形框与标签
                    label = '%s %.2f' % (
                        self.yolov7['class_name'][class_id], conf)
                    self.plot_one_box(
