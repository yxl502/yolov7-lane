# 开头导入moviepy  pip install moviepy
...
def process_an_image(img):
    ...
    # points = np.array([[(331, rows), (573, 461), (678, 453), (cols, rows)]])

    def click(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            if len(points) < 4:
                points.append((x, y))  # 只记录前四次鼠标左击的位置
...
if __name__ == "__main__":
    output = 'my_output.mp4'     #输出视频文件
    clip = VideoFileClip("demo.mp4")   #输入视频文件
    # clip = VideoFileClip("03.mov")   #输入视频文件
